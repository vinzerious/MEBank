...
namespace MEBank
{
    public partial class Withdraw : UserControl
    {
        ...
        public Withdraw(Dictionary<string, string> returnData)
        {
            InitializeComponent();
            data = returnData;
        ...
        private void getAccountList(string CustomerID)
        {
            try
            {
                ...
                dataFromSP = m.ExecuteSP("SPA_getAccountList", param, "Withdraw", "Query", dataList);
        ...
        private void getBalance(string IBANNo)
        {
            try
            {
                ...
                Main m = new Main();
                Dictionary<string, string> dataFromSP;
                dataFromSP = m.ExecuteSP("SPA_getBalance", param, "Withdraw", "Check", dataList);
        ...
        {
            if (cmbAccountList.SelectedItem == null || cmbAccountList.SelectedItem.Equals(""))
            {
            ...
            {
                MessageBox.Show("Please enter withdrawal amount", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                txtAmount.Focus();
                return;
            }
